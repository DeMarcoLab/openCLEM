...
from openlm.structures import ObjectiveSettings
...
class SMARACTObjectiveStage(ObjectiveStage):
    ...
    def set_relative_accumulation(self, onoff):
        ...
        if str(onoff) not in set(["0", "1"]):
            raise ValueError(
                "Input argument to set_relative_accumulation() "
...
#     def initialise_system_parameters(
#         self,
#         relative_accumulation=0,
#         reference_mark=0,
#         reference_hold=1000,
...
#         except Exception as e:
#             logger.error(e)
#             logger.error("Unable to send command to controller: " "{}".format(cmd))
#             raise e
#         else:
