...
class OpenLMCorrelationWidget(OpenLMCorrelationWidget.Ui_Form, QtWidgets.QWidget):
    ...
    def convert_points_to_matched_arrays(self):
        ...
        for i in range(len(points_1)):
            ...
            matched_points[i, 3] = points_2[i][1] - self.viewer.layers['image_2'].extent.world[0][1]
            matched_points[i, 4] = points_2[i][0] - self.viewer.layers['image_2'].extent.world[0][0]

        return matched_points

    def initialise_viewer(self):
        self.viewer.add_image(np.zeros((np.max([self.image_1.shape[1], self.image_2.shape[1]]), np.max([self.image_1.shape[0], self.image_2.shape[0]]))), name="blended_image", colormap="gray", visible=True)
        ...
        self.viewer.layers["blended_image"].translate=[0, self.viewer.layers['image_1'].data.shape[1] + self.viewer.layers['image_2'].data.shape[1]]
